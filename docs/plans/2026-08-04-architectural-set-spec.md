...
| 3 | **A-201 Elevations (N, E)** | True orthographic projections: silhouette, panel seam lines, config-specific openings, grade line, overall height + head/sill dimension strings, exterior finish keynotes |
| 4 | **A-202 Elevations (S, W)** | Same as A-201 for remaining views |
| 5 | **A-301 Building Section** | Cut through door axis: interior height, row band heights, wall assembly callout, foundation kit shown when configured (on/off variants), insulation/finish keynotes |
| 6 | **A-601 Schedules & Notes** | Window schedule (mark, row, qty, type fixed/operable/smart glass, nominal opening size), door schedule (main + extra door), finish schedule (exterior color, interior finish, floor), general notes |
...
`plans/index.html`: left sidebar with 6 sheet thumbnails, main pane showing the
...
2. All 6 sheets render for all 3 models across the QA matrix (below) with
...
4. Download PDF yields one 6-page vector PDF at 24"x36" landscape, selectable
   text, < 5 MB, generated in < 10 s on a mid-tier laptop.
...
| Layer | What | How |
|-------|------|-----|
| Fixtures | 12 config URLs committed to `docs/plans/archset-qa-fixtures.md`: 3 models x {default, strip, viewport, custom c–g mix, extra door, foundation off, smart glass, addons} | Open each, eyeball against configurator side-by-side |
...
| **Total** | **~11.5 d** | **~1.5–2 days of sessions** |
...
| `js/plans/sheets/{titleblock,dims,cover,floorplan,elevations,section,schedules}.js` | New: SVG generators |
...
- MEP/electrical, Title 24 / energy compliance
...
3. v2 candidates, in observed-demand order: site-plan template sheet, embedded
   engineered detail sheets (once they exist), roof/panel-layout plan, CMS notes.
