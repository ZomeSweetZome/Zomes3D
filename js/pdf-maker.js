...
export function generatePDFex() {
  ...
  // Хедер с логотипом и контактной информацией
  const header = {
      columns: [
          {
              image: './src/index/zomes_logo_loader.png', // Логотип компании
              width: 50,
              margin: [0, 10, 0, 0]
          },
          {
              text: [
                  { text: '44 (0)20 8133 2588', link: 'tel:+442081332588', fontSize: 9, margin: [10, 0, 10, 0] },
                  { text: ' www.zomes.webflow.io ', link: 'https://www.zomes.webflow.io', fontSize: 9, margin: [10, 0, 10, 0] },
                  { text: ' info@fdomes.com', link: 'mailto:info@fdomes.com', fontSize: 9, margin: [10, 0, 10, 0] }
              ],
              alignment: 'right',
              margin: [0, 10, 0, 0]
          }
  ...
  const docDefinition = {
      ...
      content: [
          ...
          {
              columns: [
                  { text: 'Office - 170', style: 'item' },
                  { text: '$ 22 950', style: 'price', alignment: 'right' }
              ]
          },
          { text: 'Windows', style: 'sectionHeader' },
          { text: 'Strip', style: 'item' },
          { text: 'Interior', style: 'sectionHeader' },
          { text: 'Wood Paneling', style: 'item' },
          { text: 'Exterior', style: 'sectionHeader' },
          { text: 'Off White', style: 'item' },
          { text: 'Add-ons', style: 'sectionHeader' },
          { text: 'Winter Package : Not included', style: 'item' },
          { text: 'Desk: Not included', style: 'item' },
          { text: 'Foundation Kit: Not included', style: 'item' },
          ...
          { text: 'Shipping Rates', style: 'sectionHeader' },
          {
              text: 'North & South America - for shipping quotes to North and South America, Australia and New Zealand please contact us directly. We will check the cost for your customization individually.',
              style: 'note'
          },
          {
              columns: [
                  { text: 'USA Northeast', style: 'item' },
                  { text: '$ 0', style: 'price', alignment: 'right' }
              ]
          },
          { text: 'Europe - for shipping quotes to Europe please contact us directly. We will check the cost for your customization individually.', style: 'note' },
          {
              columns: [
                  { text: 'England', style: 'item' },
                  { text: '$ 0', style: 'price', alignment: 'right' }
              ]
          }
          // Добавь остальные элементы таблицы аналогичным образом.
      ],
      styles: {
          sectionHeader: {
              fontSize: 12,
      ...
      images: {
          logo: 'data:image/jpeg;base64,' + getBase64Image('/mnt/data/A4 - 1.jpg') // Используй функцию для загрузки изображения
      }
  };

  // Генерация и сохранение PDF файла
  pdfMake.createPdf(docDefinition).download('zomes.pdf');
}
